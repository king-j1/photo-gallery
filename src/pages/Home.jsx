  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {selectedModel && allImages.length > 0 && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-sm z-50 overflow-y-auto p-4" onClick={closeGallery}>
          <div className="relative max-w-5xl w-full mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <button className="fixed top-4 right-4 text-white text-4xl hover:text-purple-400 z-10" onClick={closeGallery}>×</button>

            <div className="fixed top-4 left-4 flex gap-2 z-10">
              <button onClick={(e) => { e.stopPropagation(); zoomOut(); }} className="bg-black/70 hover:bg-black/90 text-white w-10 h-10 rounded-full text-xl font-bold backdrop-blur-sm">-</button>
              <button onClick={(e) => { e.stopPropagation(); zoomIn(); }} className="bg-black/70 hover:bg-black/90 text-white w-10 h-10 rounded-full text-xl font-bold backdrop-blur-sm">+</button>
            </div>

            <div
              ref={imgContainerRef}
              className="relative w-full h-[90vh] bg-black flex items-center justify-center mb-4 overflow-auto rounded-lg"
